using System;
using System.Collections.Generic;
using System.Text;

namespace Snakes_And_Ladders
{
    class UC_7TwoPlayerWinGame
    {
        public static int No_Play = 1;
        public static int LadderCount = 2;
        public static int SnakeCount = 3;
        public static int Winning_position = 100;
        public static int Position_check = 0;
        public static int Start_Position = 0;
        public static void snakeAndLadder6()
        {
            Console.WriteLine("Welcome to Snake And Ladder program");
            Boolean player1IsPlaying = true;
            int player1Position = Start_Position, player2Position = Start_Position, DiceRoll, options, DiceRolledTimes = 0;

            Random random = new Random();
            while (player1Position != Winning_position && player2Position != Winning_position)
            {
                DiceRoll = random.Next(6) + 1;
                DiceRolledTimes++;
                options = random.Next(3) + 1;

                switch (options)
                {
                    case 1:
                        if (player1IsPlaying)
                        {
                            player1Position = player1Position + 0;
                            player1IsPlaying = false;
                        }
                        else
                        {
                            player2Position = player2Position + 0;
                            player1IsPlaying = true;
                        }
                        break;

                    case 2:
                        if (player1IsPlaying)
                        {

                            if (player1Position + DiceRoll <= Winning_position)
                            {

                                player1Position = player1Position + DiceRoll;
                                Console.WriteLine("Player 1 landed on a ladder");

                            }

                        }
                        else
                        {

                            if (player2Position + DiceRoll <= Winning_position)
                            {

                                player2Position = player2Position + DiceRoll;
                                Console.WriteLine("Player 2 landed on a ladder");

                            }

                        }

                        break;

                    case 3:
                        if (player1IsPlaying)
                        {

                            player1Position = player1Position -DiceRoll;
                            player1IsPlaying = false;

                            if (player1Position < 0)
                            {

                                player1Position = 0;

                            }

                            Console.WriteLine("Player 1 landed on a snake");

                        }
                        else
                        {

                            player2Position = player2Position - DiceRoll;
                            player1IsPlaying = true;

                            if (player2Position < 0)
                            {

                                player2Position = 0;

                            }

                            Console.WriteLine("Player 2 landed on a snake");

                        }
                        break;
                    default:
                        Console.WriteLine("Enter proper value");
                        break;

                }
                Console.WriteLine("Player 1 position  = " + player1Position);
                Console.WriteLine("Player 2 position  = " + player2Position);

            }

            if (player1Position == 100)
            {

                Console.WriteLine("Player 1 won the game!");

            }
            else
            {

                Console.WriteLine("Player 2 won the game!");

            }
            Console.WriteLine("Dice was rolled " + DiceRolledTimes + " to win the game");
        }
    }
}
    

